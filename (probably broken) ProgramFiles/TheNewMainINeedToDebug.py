...
from GUI.AdminScreen import AdminScreenLayout as AS
...
class Manager(ScreenManager):
    ...
    def AdminScreen(self):
        if 'AdminScreen' in [x.name for x in self.screens]:
            self.screens.pop([x.name for x in self.screens].index('AdminScreen'))
        screen = Screen(name='AdminScreen')
        screen.add_widget(AS())
        self.add_widget(screen)
    ...
    def cheatyFunc(self):
        ...
        self.AdminScreen()
        self.current = 'AdminScreen'#Makes the basic kivy window 
Builder.load_string('''
<Screen1>:
    name: 'LoginScreen'
    Login:
