...
class CustomDropDown(BoxLayout):
      def chooseLesson(self,selected_button,accualTitleWithoutSpaces):
            reader = ce.readCSVFile('/'.join(sys.path[0].split('\\')[:-1])+'/TypingFiles/LessonList.csv')
...
class HomeWindow(GridLayout):
      ...
      def chooseLesson(self,selected_button):
            reader = ce.readCSVFile('/'.join(sys.path[0].split('\\')[:-1])+
      ...
      def fillinstats(self):
            try:
                  yesterdayStats = ce.readCSVFile('/'.join(sys.path[0].split('\\')[:-1])+'/UserData/'+self.user.username+'/history.csv')
      ...
      def findNextLesson(self,yesterdaylesson):
            lessonlist = ce.readCSVFile('/'.join(sys.path[0].split('\\')[:-1])+'/TypingFiles/LessonList.csv')
            todaylesson = ''
            todaylesson = 0
      ...
      def builddadropdown(self):
            rawdata = ce.readCSVFile('/'.join(sys.path[0].split('\\')[:-1])+'/TypingFiles/LessonList.csv')
...
<CustomDropDown>
      DropDown:
            ...
            width: 350
'''
            lessonsWithSpaces = lj(lessons,32)
